...
class _DayViewWidgetState extends State<DayViewWidget> {
  ...
  // Auto-scroll variables dengan dynamic zone calculation
  ...
  // Improved auto scroll dengan dynamic zone calculation
  void _startAutoScroll(double globalY) {
    if (_isAutoScrolling) return;

    _autoScrollTimer?.cancel();
    
    // Calculate dynamic auto scroll zone berdasarkan layout
    _calculateAutoScrollZone();
    
    // Use WidgetsBinding to safely get MediaQuery
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      
      // Get app bar height dan SafeArea untuk akurasi
      double appBarHeight = kToolbarHeight;
      double statusBarHeight = MediaQuery.of(context).padding.top;
      double fullDayEventsHeight = _fullDayEvents.isNotEmpty ? 
          (_fullDayEvents.length * 25.0) + 40.0 : 0.0;
      
      // Calculate effective screen bounds untuk time events area
      double effectiveTop = statusBarHeight + appBarHeight + fullDayEventsHeight;
      double effectiveBottom = MediaQuery.of(context).size.height - 100;
      
      // Check scroll zones relative to time events area
      bool shouldScrollUp = globalY < (effectiveTop + _currentAutoScrollZone);
      bool shouldScrollDown = globalY > (effectiveBottom - _currentAutoScrollZone);
      ...
      _autoScrollTimer = Timer.periodic(const Duration(milliseconds: 16), (timer) {
        ...
        if (shouldScrollUp) {
          newOffset = (currentOffset - _autoScrollSpeed).clamp(
            0.0, 
  ...
  // Calculate dynamic auto scroll zone
  void _calculateAutoScrollZone() {
    double adjustment = _fullDayEvents.length * 5.0;
    _currentAutoScrollZone = (_baseAutoScrollZone + adjustment).clamp(60.0, 120.0);
  }
          ...
          return DragTarget<CalendarEvent>(
            onMove: (details) {
              try {
                ...
                // Improved auto-scroll logic with better coordinates
                if (_isDragging) {
                  double globalY = details.offset.dy;
                  _startAutoScroll(globalY);
                }

                // Convert global position ke local position untuk event column
  ...
  void _createEventAtPosition(double yPosition) {
    double hours = yPosition / _hourHeight;
    int targetHour = hours.floor();
    // ✅ CHANGED: Set menit ke 0 untuk jam penuh, tidak pakai snap interval
    int targetMinute = 0;
    ...
    // ✅ CHANGED: Buat event dengan jam penuh (start jam X:00, end jam X+1:00)
    DateTime baseDate = DateTime(
      widget.date.year,
      widget.date.month,
      widget.date.day,
            ...
            builder: (context, constraints) {
              return Container(
                constraints: BoxConstraints(
                  maxWidth: MediaQuery.of(context).size.width - _timeColumnWidth - 40,
                ),
